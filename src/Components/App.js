...
class App extends Component {
  ...
  onDateChange = (date, dateString) => {    //func takes the date changes in the format of the firebase nodes and execute the fuction for fetching the devices for the table
  ...
  onChangeRadio = e => {     // this for the radio buttons to select weekly monthly or the range report
    if (e.target.value === 1) { 
  ...
  previousDayButton() {   //this si the button near date sleector and whenever it cahges the date by one day and fetch the data
  ...
  nextDayButton() {   //it changes the day by one day and fetch the data
  ...
  selectDateReportMonthly() {   //this is to geterate repot monthly it takes current date and convert it into an array of last one month and send it to fetch report
    var dateList = [];
  ...
  selectDateReportWeekly() {    //this for genrating weekly reports
    var dateList = [];
    var todaysDate = moment(new Date()).subtract(1, "days");
  ...
  footFallClick() {      //when tis is clicked it is passed onto the graph component and graph is chenged
        ...
        <div className="main" style={{backgroundColor:this.state.backgroundColor}}>
          ...
          >
            ...
            <div style={{  padding: 10 }}>
              ...
              <DeviceTable getData={this.getTableData} />
            </div>
            <Button
              onClick={() => {
